  ...
  <tr class="border-b hover:bg-gray-50">
    <td class="px-4 py-2">
      <button onclick="duzenle(${cari.id})" class="text-blue-600 hover:underline text-left">
        ${escapeHtml(cari.unvan)}
      </button>
    </td>
   
    <td class="px-4 py-2">${CARI_TIPI_LABELS[cari.cariTipi]}</td>
    <td class="px-4 py-2">${escapeHtml(cari.sehir)}</td>
...
document.getElementById("cariTipiFiltre").innerHTML +=
  enumOptions(CARI_TIPI_LABELS);
document.getElementById("cariDurumuFiltre").innerHTML +=
  enumOptions(DURUM_LABELS);

document.getElementById("arama-alani").addEventListener("input", renderList);
document
  .getElementById("cariTipiFiltre")
  .addEventListener("change", renderList);
document
  .getElementById("cariDurumuFiltre")
  .addEventListener("change", renderList);
